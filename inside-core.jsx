...
const T = {
  negro: '#1F1D1A', negro80: '#4B4845', negro50: '#8F8D8C', negro20: '#CCCAC9',
  blanco: '#F5F3F2', beige0: '#FAF8F7', beige1: '#EBE3DB', beige2: '#D4C3B3',
  acento: '#8E2D44', acentoDark: '#6B1F32', acentoLight: '#C06078',
...
const Kicker = ({ children, color = T.acento, style = {} }) => (
  <span style={{
...
const Ph = ({ label = 'Imagen pendiente', note, ratio = '16 / 10', dark = false,
             radius = 4, video = false, style = {}, children }) => {
    ...
    <div style={{
      ...
      {video ? (
        <div style={{ position: 'relative', zIndex: 1, display: 'flex',
          ...
          <div style={{ width: '74px', height: '74px', borderRadius: '9999px',
            background: T.acento, display: 'flex', alignItems: 'center',
            justifyContent: 'center', boxShadow: '0 8px 30px rgba(31,29,26,0.28)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '12px' }}>
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none"
            stroke={ink} strokeWidth="1.4" style={{ marginBottom: '8px' }}>
            <rect x="3" y="5" width="18" height="14" rx="1.5" />
            <circle cx="9" cy="10" r="1.6" />
            <path d="M3 17l5-5 4 3.5L16 11l5 5" />
          </svg>
          <div style={{ fontFamily: T.sans, fontSize: '10px', fontWeight: 500,
            letterSpacing: '0.16em', textTransform: 'uppercase', color: ink }}>
            {label}
          </div>
          {note && <div style={{ fontFamily: T.sans, fontSize: '10px',
            fontWeight: 300, color: ink, marginTop: '3px', opacity: 0.8 }}>{note}</div>}
        </div>
      )}
      {children}
    </div>
...
const TextPh = ({ lines = 3, w = '100%' }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: '9px', maxWidth: w }}>
    {Array.from({ length: lines }).map((_, i) => (
      <div key={i} style={{ height: '11px', borderRadius: '3px',
...
const BrandIcon = ({ n = 1, size = 30, color = T.negro }) => {
  const src = `assets/icons/icono-${n}.svg`;
  return <img src={src} width={size} height={size} alt=""
    style={{ display: 'block', filter: color === T.blanco ? 'invert(1)' : 'none' }} />;
};
...
const EDITION = { num: 'Nº1', date: '9 junio 2026', name: 'Activum Inside' };
...
const SECTIONS = [
  ...
  {
    id: 'wellbeing', num: '03', kind: 'wellbeing', fixed: true, icon: 4,
    section: 'Wellbeing',
  ...
  {
    id: 'cuerpo', num: '04', kind: 'cuerpo', fixed: true, icon: 4,
  ...
  {
    id: 'beneficios', num: '05', kind: 'beneficios', fixed: false, icon: 5,
  ...
  {
    id: 'primera', num: '06', kind: 'entrevista', fixed: true, icon: 1,
  ...
  {
    id: 'bitabit', num: '07', kind: 'bitabit', fixed: true, icon: 2,
    section: 'Bit a Bit',
  ...
  {
    id: 'galeria', num: '08', kind: 'galeria', fixed: true, icon: 3,
  ...
  {
    id: 'voz', num: '09', kind: 'buzon', fixed: true, icon: 4,
    section: 'Activum Lab',
...
const PROMOS = [
  {
    id: 'lanucia', kind: 'promo', section: 'La Nucía One', promo: true, icon: 3,
...
const Slot = ({ id, ratio = '16 / 10', label = 'Sube una imagen', dark = false,
               radius = 4, shape = 'rounded', style = {} }) =>
  React.createElement('image-slot', {
    id, shape, radius: String(radius), fit: 'cover', placeholder: label,
    style: {
      display: 'block', width: '100%', height: 'auto', aspectRatio: ratio,
